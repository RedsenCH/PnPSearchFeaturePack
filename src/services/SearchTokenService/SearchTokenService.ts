...
const SearchTokenService_ServiceKey = "ModernSearchSearchTokenService";

export enum BuiltinSearchTokenNames {
    /**
...
export class SearchTokenService implements ITokenService {
    ...
    private tokenValuesList: { [key: string]: any } = {
        [BuiltinSearchTokenNames.inputQueryText]: undefined,
        [BuiltinSearchTokenNames.originalInputQueryText]: undefined,
        [BuiltinSearchTokenNames.searchTerms]: undefined,
        [BuiltinSearchTokenNames.filters]: undefined,
        [BuiltinSearchTokenNames.verticals]: undefined,
    };
    ...
    public constructor(serviceScope: ServiceScope) {
        ...
        serviceScope.whenFinished(() => {
            this.dateHelper = serviceScope.consume<DateHelper>(
    ...
    public setTokenValue(token: string, value: any) {
        ...
        if (Object.keys(this.tokenValuesList).indexOf(token) !== -1) {
            ...
            Log.warn(
                SearchTokenService_ServiceKey,
                `The token '${token}' not allowed.`
            );
        }
    }
        ...
        if (inputString) {
            ...
            if (tokens !== null && tokens.length > 0) {
                tokens.forEach((token) => {
                    ...
                    // 'undefined' => token hasn't been initialized in the SearchTokenService instance. We left the token expression untouched (ex: {token}). Ex: no filters Web Part connected, etc.
                    ...
                    const tokenValue = ObjectHelper.byPath(
                        this.tokenValuesList,
                        tokenName
                    );
                    ...
                    if (tokenValue !== undefined) {
                        if (tokenValue !== null) {
                            inputString = inputString.replace(
                                new RegExp(
                                    StringHelper.escapeRegExp(token),
                                    "gi"
                                ),
                                tokenValue
                            );
    ...
    public async getPageProperties(): Promise<any> {
        ...
        if (this.pageContext.listItem) {
            const url =
                this.pageContext.web.absoluteUrl +
                `/_api/web/GetList(@v1)/RenderExtendedListFormData(itemId=${this.pageContext.listItem.id},formId='viewform',mode='2',options=7)?@v1='${this.pageContext.list.serverRelativeUrl}'`;
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    item = Object.keys(itemRow.Data.Row[0]).reduce(
                        (c: any, k: any) => (
                            (c[k] = itemRow.Data.Row[0][k]), c
                        ),
                        {}
                    );
                } else {
    ...
    public async getUserProfileProperties(): Promise<IProfileProperties> {
        ...
        if (response.ok) {
            ...
            const errorMessage = `${SearchTokenService_ServiceKey}: Error retrieving user profiel properties. Details: ${
            ...
            Log.error(SearchTokenService_ServiceKey, error, this.serviceScope);
            throw error;
        }
    }
            ...
            while (matches !== null && item != null) {
                ...
                if (/\.Label|\.TermID/gi.test(pageProperty)) {
                    ...
                    if (
                        ...
                        itemProp = item[columnName]
                            .map((taxonomyValue: any) => {
                    ...
                    itemProp = ObjectHelper.byPath(
                        item,
                        pageProperty.toLowerCase()
                    );
                }
        ...
        if (matches !== null) {
            while (matches !== null) {
                ...
                inputString = inputString.replace(
                    ...
                    this.pageContext
                        ? ObjectHelper.byPath(this.pageContext, prop)
                        : ""
                );
    ...
    private async replaceUserTokens(inputString: string): Promise<string> {
        ...
        while (matches !== null) {
            ...
            if (!/^(name|email)$/gi.test(userProperty)) {
                ...
                propertyValue = ObjectHelper.byPath(
                    this.userProperties,
                    `"${userProperty}"`
                );
            } else {
    ...
    private replaceDateTokens(inputString: string): string {
        ...
        while ((match = results.exec(inputString)) !== null) {
            for (const result of match) {
                ...
                const digit =
                    parseInt(
                        operatorSplit[operatorSplit.length - 1]
                            .replace("{", "")
                            .replace("}", "")
                            .trim()
                    ) * addOrRemove;
     ...
     * @param inputString the input string containing tokens
     */
    public replaceQueryStringTokens(inputString: string) {
        ...
        if (matches !== null) {
            ...
            while (matches !== null) {
                ...
                const itemProp = decodeURIComponent(
                    queryParameters.get(qsProp) || ""
                );
                if (itemProp) {
    ...
    private replaceWebTokens(inputString: string): string {
        ...
        if (matches !== null) {
            while (matches !== null) {
                ...
                inputString = inputString.replace(
                    new RegExp(matches[0], "gi"),
                    this.pageContext.web
                        ? (this.pageContext.web as any)[webProp]
                        : ""
                );
    ...
    private replaceSiteTokens(inputString: string): string {
        ...
        if (matches !== null) {
            while (matches !== null) {
                ...
                const sitePropertyValue = ObjectHelper.byPath(
                ...
                if (sitePropertyValue) {
                    inputString = inputString.replace(
                        new RegExp(matches[0], "gi"),
                        this.pageContext.site ? sitePropertyValue : ""
                    );
                }
            ...
            while (matches !== null) {
                ...
                inputString = inputString.replace(
                    ...
                    this.pageContext.list
                        ? ObjectHelper.byPath(this.pageContext.list, listProp)
                        : ""
                );
    ...
    private replaceLegacyPageContextTokens(inputString: string): string {
        ...
        if (matches !== null) {
            while (matches !== null) {
                ...
                inputString = inputString.replace(
                    new RegExp(matches[0], "gi"),
                    this.pageContext.legacyPageContext
                        ? ObjectHelper.byPath(
                              this.pageContext.legacyPageContext,
                              legacyProp
                          )
    ...
    public async getHubInfo(): Promise<any> {
        try {
            ...
            const data = await this.spHttpClient.get(
                restUrl,
                SPHttpClient.configurations.v1,
                {
                    headers: {
            ...
            Log.error(
                SearchTokenService_ServiceKey,
                new Error(
                    `Error while fetching Hub site data. Details: ${error}`
                ),
                this.serviceScope
            );
            return null;
     ...
     */
    private recursivelyLowercaseJSONKeys(obj: any): any {
        const copyOfObj: any = obj;
        ...
        return Object.keys(copyOfObj).reduce((prev: any, curr: string) => {
            prev[curr.toLowerCase()] = this.recursivelyLowercaseJSONKeys(
                copyOfObj[curr]
            );
            return prev;
        }, {});
