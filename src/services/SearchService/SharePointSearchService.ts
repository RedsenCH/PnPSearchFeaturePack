...
} from "../../models/search/ISharePointSearchResults";
...
import {
    ISuggestResult,
    ISuggestQuery,
} from "../../models/search/ISuggestQuery";
import {
...
const AvailableQueryLanguages_StorageKey =
    "pnpSearchResults_AvailableQueryLanguages";

export class SharePointSearchService implements ISharePointSearchService {
    public static ServiceKey: ServiceKey<ISharePointSearchService> =
        ServiceKey.create(SearchService_ServiceKey, SharePointSearchService);
    ...
    public async search(
    ...
    ): Promise<ISharePointSearchResults> {
        const results: ISharePointSearchResults = {
        ...
        try {
            const response: any = await this.spHttpClient.post(
                this.searchEndpointUrl,
                SPHttpClient.configurations.v1,
                {
                    ...
                    headers: {
                        ...
                        "X-ClientService-ClientTag":
                            Constants.X_CLIENTSERVICE_CLIENTTAG,
            ...
            if (response.ok) {
                const searchResponse: ISharePointSearchResponse =
                    await response.json();
                ...
                if (searchResponse.PrimaryQueryResult) {
                    ...
                    const properties =
                        searchResponse.PrimaryQueryResult.RelevantResults.Properties.filter(
                            (property) => {
                    ...
                    const resultRows =
                        searchResponse.PrimaryQueryResult.RelevantResults.Table
                            .Rows;
                    const refinementResultsRows =
                        searchResponse.PrimaryQueryResult.RefinementResults;
                    ...
                    refinementRows.forEach((refiner: any) => {
                        ...
                        refiner.Entries.forEach((item: any) => {
                            values.push({
                                ...
                                name: item.RefinementValue.replace(
                                    "string;#",
                                    ""
                    ...
                    results.totalRows =
                        searchResponse.PrimaryQueryResult.RelevantResults.TotalRows;

                    if (!isEmpty(searchResponse.SpellingSuggestion)) {
                        results.spellingSuggestion =
                            searchResponse.SpellingSuggestion;
                    }
                }

                // Query rules handling
                if (searchResponse.SecondaryQueryResults) {
                    const secondaryQueryResults =
                        searchResponse.SecondaryQueryResults;

                    if (
                        Array.isArray(secondaryQueryResults) &&
                        secondaryQueryResults.length > 0
                    ) {
                        const promotedResults: ISharePointSearchPromotedResult[] =
                            [];
                        const secondaryResults: ISharePointSearchResultBlock[] =
                            [];
                        ...
                        secondaryQueryResults.forEach((e) => {
                            ...
                            if (e.SpecialTermResults) {
                                ...
                                e.SpecialTermResults.Results.forEach(
                                    (result: any) => {
            ...
            Log.error(
                "[SharePointSearchService.search()]",
                error,
                this.serviceScope
            );
            throw error;
     ...
     */
    public async getAvailableManagedProperties(): Promise<
        ISharePointManagedProperty[]
    > {
        const managedProperties: ISharePointManagedProperty[] = [];
        const searchQuery: ISharePointSearchQuery = {};

        searchQuery.Querytext = "*";
        searchQuery.Refiners =
            "ManagedProperties(filter=50000/0/*,sort=name/ascending)";
        ...
        try {
            ...
            Log.error(
                "[SharePointSearchService.getAvailableManagedProperties()]",
                error,
                this.serviceScope
            );
            throw error;
    ...
    public async getAvailableQueryLanguages(): Promise<any> {
        try {
            ...
            let availableLanguages = this.clientStorage.local.get(
                AvailableQueryLanguages_StorageKey
            );
            ...
            if (availableLanguages) {
                ...
                const response: any = await this.spHttpClient.get(
                    `${this.pageContext.web.absoluteUrl}/_api/web/RegionalSettings/InstalledLanguages`,
                    SPHttpClient.configurations.v1,
                    {
                        headers: {
                            "X-ClientService-ClientTag":
                ...
                if (response.ok) {
                    ...
                    this.clientStorage.local.put(
                        AvailableQueryLanguages_StorageKey,
                        availableLanguages.Items,
                        dateAdd(new Date(), "week", 1)
                    );
            ...
            Log.error(
                "[SharePointSearchService._getQueryLanguages()]",
                error,
                this.serviceScope
            );
    ...
    public async suggest(query: string): Promise<string[]> {
        ...
        const searchSuggestQuery: ISuggestQuery = {
            ...
            prefixMatch: true,
            culture: LocalizationHelper.getLocaleId(
                this.pageContext.cultureInfo.currentUICultureName
            ).toString(),
        ...
        try {
            ...
            if (response.ok) {
                const suggestionResponse: ISuggestResult =
                    await response.json();
            ...
            Log.error(
                "[SharePointSearchService.suggest()]",
                error,
                this.serviceScope
            );
            throw error;
    ...
    private getSearchResults(resultRows: any): ISharePointSearchResult[] {
        // Map search results
        const searchResults: ISharePointSearchResult[] = resultRows.map(
            (elt: any) => {
    ...
    private getRequestPayload(searchQuery: ISharePointSearchQuery): string {
        const queryPayload: any = cloneDeep(searchQuery);

        queryPayload.HitHighlightedProperties = this.fixArrProp(
            queryPayload.HitHighlightedProperties
        );
        queryPayload.Properties = this.fixArrProp(queryPayload.Properties);
        queryPayload.RefinementFilters = this.fixArrProp(
            queryPayload.RefinementFilters
        );
        queryPayload.ReorderingRules = this.fixArrProp(
            queryPayload.ReorderingRules
        );
        queryPayload.SelectProperties = this.fixArrProp(
            queryPayload.SelectProperties
        );
