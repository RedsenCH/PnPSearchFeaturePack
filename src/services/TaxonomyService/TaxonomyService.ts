...
export class TaxonomyService implements ITaxonomyService {
    public static ServiceKey: ServiceKey<ITaxonomyService> = ServiceKey.create(
        TaxonomyService_ServiceKey,
        TaxonomyService
    );
    ...
    public async getTermsById(
        ...
        if (response.status === 200) {
            ...
            const termStoreResultTerms: ITerms[] = responseJson.filter(
                (r: { [x: string]: string }) =>
                    r["_ObjectType_"] === "SP.Taxonomy.TermCollection"
            );
