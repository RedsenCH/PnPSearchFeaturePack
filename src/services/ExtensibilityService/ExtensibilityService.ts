...
export class ExtensibilityService {
    ...
    public static ServiceKey: ServiceKey<IExtensibilityService> =
        ServiceKey.create(
            ExtensibilityService_ServiceKey,
     ...
     */
    public async loadExtensibilityLibraries(
        librairiesConfiguration: IExtensibilityConfiguration[]
    ): Promise<IExtensibilityLibrary[]> {
        const extensibilityLibraries: IExtensibilityLibrary[] = [];

        try {
            // Load only "Enabled" configuration
            const promises = librairiesConfiguration
                .filter((configuration) => configuration.enabled)
                .map((configuration) => {
                    return new Promise<any>((resolve, reject) => {
                        return SPComponentLoader.loadComponentById(
                            configuration.id
                        )
                            .then((extensibilityLibraryComponent: any) => {
                                ...
                                const libraryMainEntryPoints = Object.keys(
                                    extensibilityLibraryComponent
                                ).filter((property) => {
                                    ...
                                    const extensibilityLibraryPrototype: IExtensibilityLibrary =
                                        extensibilityLibraryComponent[property]
                                            .prototype;
                                    return (
                                        property.indexOf("__") === -1 &&
                                ...
                                if (libraryMainEntryPoints.length === 1) {
                                    if (
                                        ...
                                        extensibilityLibrary =
                                            new extensibilityLibraryComponent[
                                                libraryMainEntryPoints[0]
                                            ]();
                                    }
                                }
                                ...
                                Log.verbose(
                                    ExtensibilityService_ServiceKey,
                                    `Extensibility library component with id '${configuration.id}' and name '${libraryMainEntryPoints[0]}' loaded.`,
                                    this.serviceScope
                                );
                                resolve(
                                    extensibilityLibrary as IExtensibilityLibrary
                                );
                            })
                            .catch((error) => {
                                Log.warn(
                                    ExtensibilityService_ServiceKey,
                                    `No extensibility library component found with id '${configuration.id}'`,
                                    this.serviceScope
                                );
                                resolve(undefined);
                            });
