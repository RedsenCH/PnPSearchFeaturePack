...
export class HorizontalFilterLayout extends BaseLayout<IHorizontalLayoutProperties> {
    public async onInit(): Promise<void> {
        ...
        this.properties.preferedFilterNumberPerRow = this.properties
            .preferedFilterNumberPerRow
            ? this.properties.preferedFilterNumberPerRow
            : 3;
        this.properties.columnSizePercentage = this.properties
            .columnSizePercentage
            ? this.properties.columnSizePercentage
            : 33;
    }
    ...
    public getPropertyPaneFieldsConfiguration(
        ...
        return [
            PropertyPaneSlider("layoutProperties.preferedFilterNumberPerRow", {
                label: commonStrings.Layouts.Horizontal
                    .PreferedFilterNumberPerRow,
                min: 1,
                max: 6,
                step: 1,
    ...
    public onPropertyUpdate(
        ...
        if (
            propertyPath.localeCompare(
                "layoutProperties.preferedFilterNumberPerRow"
            ) === 0
        ) {
            // Calculate the correct % for card flex-basis
            this.properties.columnSizePercentage =
                Math.floor(100 / newValue) - 1;
        }
    }
}
