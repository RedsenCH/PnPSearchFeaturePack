...
import { BuiltinSearchTokenNames } from "../services/SearchTokenService/SearchTokenService";
    ...
    public static getAppliedFiltersFromTokens(
        ...
        if (tokens !== null && tokens.length > 0) {
            tokens.forEach((token) => {
                ...
                if (
                    tokenName.indexOf(BuiltinSearchTokenNames.filters) !== -1 &&
                    tokenParts.length > 1
                ) {
                    ...
                    const selectedFiltersByName = selectedFilters.filter(
                        (selectedFilter) =>
                            selectedFilter.filterName === tokenParts[1]
                    );
            ...
            if (filter.values.length > 1) {
                        ...
                        if (moment(value, moment.ISO_8601, true).isValid()) {
                            if (
                                ...
                                (filterValue.operator ===
                                    FilterComparisonOperator.Geq ||
                                    filterValue.operator ===
                                        FilterComparisonOperator.Gt)
                            ) {
                                startDate = value;
                            }
                            ...
                            if (
                                !endDate &&
                                (filterValue.operator ===
                                    FilterComparisonOperator.Lt ||
                                    filterValue.operator ===
                                        FilterComparisonOperator.Leq)
                            ) {
                                endDate = value;
                            }
                        }
                ...
                if (startDate && endDate) {
                    refinementQueryConditions.push(
                        `${filter.filterName}:range(${startDate},${endDate})`
                    );
                } else {
                    refinementQueryConditions.push(
                ...
                if (filter.values.length === 1) {
                    ...
                    if (
                        moment(refinementToken, moment.ISO_8601, true).isValid()
                    ) {
                        if (
                            filterValue.operator ===
                                FilterComparisonOperator.Gt ||
